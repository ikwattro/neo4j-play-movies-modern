...
CREATE FULLTEXT INDEX Person
FOR (n:Person)
ON EACH[n.name, n.bornIn];

// Create VECTOR indexes

CREATE VECTOR INDEX actorsV IF NOT EXISTS
FOR (n:Person)
ON n.embedding
OPTIONS { indexConfig: {
 `vector.dimensions`: 3,
...
SET Keanu.embedding = vector([3, 4, 5], 3, INTEGER NOT NULL)
...
SET TomC.embedding = vector([3, 4, 5], 3, INTEGER NOT NULL)
